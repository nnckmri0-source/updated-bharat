...
// ----------------------------------------------------------------------------
// EMAIL-ONLY login: { email, password } is verified against Firebase Auth and
// the email must be in the admin allowlist ("admin-emails" node — currently
// ONLY nnckmri0@gmail.com). No other login method exists.

import { NextRequest, NextResponse } from "next/server";
import { cookies } from "next/headers";
import { throwIfResponse, jsonError } from "@/lib/api-utils";
import { ADMIN_READY } from "@/lib/firebase-admin";
import { createSessionToken, verifyAdminRequest, ADMIN_COOKIE } from "@/lib/admin-auth";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!email || !isEmail(email)) return jsonError("Admin email required — login is email-only", 400);

    let displayName: string | null = null;

    // ----- Firebase Auth email login (the ONLY login method) -----
